	...
	namespace SSDServing {
        namespace SPFresh {
            ...
            double SearchSequential(SPANN::Index<ValueType>* p_index,
                ...
                int p_maxQueryCount, int p_internalResultNum, int currentVectorNum)
            {
                ...
                auto func = [&]()
                {
                    ...
                    while (true)
                    {
                        ...
                        if (index < numQueries)
                        {
                            ...
                            p_index->SearchDiskIndex(p_results[index], currentVectorNum, &(p_stats[index]));
                ...
                bool showStatus = true)
            {
                ...
                for (int i = 0; i < avgStatsNum; i++)
                {
                    ...
                    totalQPS += SearchSequential(p_index, numThreads, results, stats, queryCountLimit, internalResultNum, currentVectorNum);
                ...
                if (p_opts.m_calTruth)
                {
                    if (p_opts.m_searchResult.empty()) {
                        ...
                        std::string path = "/home/lyh/RecallResults/SPFresh/Recall.csv";
                        std::ofstream file(path, std::ios::app);
                        int truthK = p_opts.m_resultNum;
                        LoadTruth(p_opts, truth, numQueries, truthFileName, truthK);
                        float recall = CalculateRecallSPFresh<ValueType>((p_index->GetMemoryIndex()).get(), results, truth, p_opts.m_resultNum, truthK, querySet, vectorSet, numQueries);
                        file<<currentVectorNum<<","<<recall<<"\n";
                    } else {
            ...
            void ConcurrentUpdate(SPANN::Index<ValueType>* p_index)
            {
                ...
                if (!p_opts.m_fullVectorPath.empty() && fileexists(p_opts.m_fullVectorPath.c_str())) {//BuildSSDIndex -> FullVectorPath
                            ...
                            do {
                                ...
                                if (insert_status == std::future_status::timeout && notSearch) {
                                    ...
                                    if (p_opts.m_searchDuringUpdate/*BuildSSDIndex -> SearchDuringUpdate*/){
                                        ...
                                        //p_index->StopMerge();
                                        pause_threads();
                                        read_num_snapshot = p_index->GetNumSamples();
                    ...
                    //p_index->GetDiskIndex()->SavePostingSizesAndVersionMaps();

                    //to be modified, p_index->GetMemoryIndex() is the memory index still includes the deleted data
                    //p_index->GetDiskIndex()->BuildIndex(vectorReader, 0, baseVectorSplitList.at(p_opts.m_ternimateNum)+1, p_index->GetMemoryIndex(), *p_index->GetOptions(), *p_index->GetVersionLabel(), *p_index->GetPostingVersionLabel());
