...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        */
        ...
        private:
            std::string name = "Data";
            SizeType rows = 0;//data entry num
            DimensionType cols = 1;//each data entry total size(total size = metadata size + data num * each data size)
            char* data = nullptr;//data first address
            bool ownData = false;
            SizeType incRows = 0;//rows num that already be written to blocks
            SizeType maxRows;//the maximum of data entries
            SizeType rowsInBlock;//the entry num of each block that could be stored. Each block could contain multiple entries [0, rowsInBlock]
            SizeType rowsInBlockEx;//the exponent of each block size
            std::shared_ptr<std::vector<char*>> incBlocks;//each block address, incBlocks.size() is the block num

            //a data entry is consist of: [metadata, raw data]
            DimensionType colStart = 0;//raw data beginning postion of a single data entry, the metadata size
            DimensionType mycols = 0;//data num of a single data entry
...
#undef GETITEM
            ...
            ErrorCode AddBatch(SizeType num, const T* pData = nullptr)//parameter num is the data row number in pData
            {
                if (colStart != 0) return ErrorCode::Success;//if the metadata size is not equal to 0
                if (R() > maxRows - num) return ErrorCode::MemoryOverFlow;

                SizeType written = 0;
                while (written < num) {
                    SizeType curBlockIdx = ((incRows + written) >> rowsInBlockEx);// get block's id
                    if (curBlockIdx >= (SizeType)(incBlocks->size())) {
                    ...
                    SizeType curBlockPos = ((incRows + written) & rowsInBlock);//get current block offset
                    SizeType toWrite = min(rowsInBlock + 1 - curBlockPos, num - written);
                    if (pData) {
