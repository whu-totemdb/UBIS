...
IniReader::GetParameter(const std::string& p_section, const std::string& p_param, const DataType& p_defaultVal) const
{
    ...
    {
        //LOG(Helper::LogLevel::LL_Info, "p_section = %s, p_param=%s\n", p_section.c_str(), p_param.c_str());
        return p_defaultVal;
    }

    //LOG(Helper::LogLevel::LL_Info, "p_section = %s, p_param = %s, value = %s\n", p_section.c_str(), p_param.c_str(), value.c_str());
...
IniReader::ConvertStringTo(std::string&& p_str, const DataType& p_defaultVal)
{
    ...
    {
        //LOG(Helper::LogLevel::LL_Info, "p_str = %s, value = %s\n", p_str.c_str(), value);
        return value;
    }
