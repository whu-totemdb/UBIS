...
ErrorCode
VectorIndex::SaveIndex(const std::string& p_folderPath, const SPTAG::SPANN::Options& p_opts)
{
    if (!m_bReady || GetNumSamples() - GetNumDeleted() == 0) return ErrorCode::EmptyIndex;
    ...
    if (NeedRefine()) 
    {
        ret = RefineIndex(handles, p_opts, nullptr);
    }
    else 
    {
        if (m_pMetadata != nullptr) ret = m_pMetadata->SaveMetadata(handles[metaStart], handles[metaStart + 1]);
...
#undef DefineVectorValueType
                    ...
                    for (int i = 0; i < candidateNum && currReplicaCount < replicaCount; ++i)
                    {
                        ...
                        {
                            if(i == 0) noresultCount.fetch_add(1);
                            break;
                        }
    ...
    LOG(Helper::LogLevel::LL_Info, "Searching replicas ended. RNG failed count: %llu, notAppendVectorNum: %d\n", static_cast<uint64_t>(rngFailedCountTotal.load()), noresultCount.load());
}
#endif
