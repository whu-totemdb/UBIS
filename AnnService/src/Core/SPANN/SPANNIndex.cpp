...
namespace SPTAG
{
    namespace SPANN
    {
        ...
        ErrorCode Index<T>::LoadIndexDataFromMemory(const std::vector<ByteArray>& p_indexBlobs)
        {
            ...
            if (!m_extraSearcher->LoadIndex(m_options, m_versionMap, m_postingVersionMap)) return ErrorCode::Fail;
        ...
        ErrorCode Index<T>::LoadIndexData(const std::vector<std::shared_ptr<Helper::DiskIO>>& p_indexStreams)
        {
            ...
            if (m_options.m_useSPDK) {
                ...
                m_postingVersionMap.Initialize(m_options.m_vectorSize, m_index->m_iDataBlockSize, m_index->m_iDataCapacity);//to be modified
                ...
                if (!storeExtraSearcher->LoadIndex(m_options, m_versionMap, m_postingVersionMap)) {
            ...
            /*else {
                ...
                m_postingVersionMap.Load(m_options.m_postingVersionLabelFile, m_index->m_iDataBlockSize, m_index->m_iDataCapacity);
            }*/
...
#undef DefineSSDParameter
        ...
        ErrorCode Index<T>::SaveIndexData(const std::vector<std::shared_ptr<Helper::DiskIO>>& p_indexStreams)
        {
            ...
            m_postingVersionMap.Save(m_options.m_postingVersionLabelFile);
            return ErrorCode::Success;
        }
...
#pragma region K-NN search
        ...
        ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
        {
            ...
            if (m_extraSearcher != nullptr) {
                ...
                //m_workspace->m_postingIDs stores the target posting ids
                //the following function searches the disk vector data and get the top k(m_options.m_searchInternalResultNum) nearest results
        ...
        template <typename T>
        ErrorCode Index<T>::SearchDiskIndex(QueryResult& p_query, int maxVectorId, SearchStats* p_stats) const
        {
            if (nullptr == m_extraSearcher) return ErrorCode::EmptyIndex;
            ...
            if (m_vectorTranslateMap.get() != nullptr) p_queryResults->Reverse();
            m_extraSearcher->SearchIndex(m_workspace.get(), *p_queryResults, m_index, p_stats, maxVectorId);
            p_queryResults->SortResult();
            return ErrorCode::Success;
        }
...
#pragma endregion
        ...
        void Index<T>::SelectHeadDynamically(const std::shared_ptr<COMMON::BKTree> p_tree, int p_vectorCount, std::vector<int>& p_selected) {
            ...
            for (int select = 2; select <= m_options.m_selectThreshold; ++select)
            {
                ...
                while (l < r - 1)
                {
                    ...
                    double diff = static_cast<double>(p_selected.size()) / p_vectorCount - m_options.m_ratio;//m_ratio is the target ratio of postings. e.g, 0.1 means it hopes to select 0.1*p_vectorCount postings
        ...
        ErrorCode Index<T>::BuildIndexInternal(std::shared_ptr<Helper::VectorSetReader>& p_reader) {
            ...
            auto t1 = std::chrono::high_resolution_clock::now();
            //select head vectors and store them in two files:m_options.m_indexDirectory + FolderSep + m_options.m_headVectorFile, m_options.m_indexDirectory + FolderSep + m_options.m_headIDFile
            //the first file stores the raw selected vector data and the other stores the ids
            ...
            if (m_options.m_buildHead) {
                ...
                }
                {
                    ...
                    //the head vector data is used to build an independent index, which is used for memory cluster-based index
                    ...
                    //store the memory index in the folder m_options.m_indexDirectory + FolderSep + m_options.m_headIndexFolder
                ...
                m_index.reset();
                if (LoadIndex(m_options.m_indexDirectory + FolderSep + m_options.m_headIndexFolder, m_index) != ErrorCode::Success) {//check if the memory posting index is built
            ...
            if (m_options.m_enableSSD) {
                ...
                //load the head index as memory index, m_index is the memory index
                ...
                //initialize ssd index object(the key members include the db and the m_postingSizeLimit)
                if (m_options.m_useKV)
                {
                    if (m_options.m_inPlace) {
                        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(m_options.m_KVPath.c_str(), m_options.m_dim, INT_MAX, m_options.m_useDirectIO, m_options.m_latencyLimit, m_options.m_mergeThreshold));
                ...
                if (m_options.m_buildSsdIndex) {
                    ...
                    //p_reader is the raw reader object that holds access to all the vector data
                    //m_index is the memory head inex
                    //m_versionMap and m_extraSearcher->m_versionMap are the same
                    //Assign each data point in the dataset to the closest posting, store the posting data to the storage engine(rocksdb) and store the postingSizes and versionMap to ssd
                    if (!m_extraSearcher->BuildIndex(p_reader, m_index, m_options, m_versionMap, m_postingVersionMap)) {
                ...
                //load the data files in the disk and initilize some data members
                if (!m_extraSearcher->LoadIndex(m_options, m_versionMap, m_postingVersionMap)) {
            ...
            m_bReady = true;
            return ErrorCode::Success;
            //after this function, m_index is the cluster-based BKT index in memory
            //m_extraSearcher is the controller that holds the disk data(postingSizes, postings' raw data, vectors' version map)
        }
