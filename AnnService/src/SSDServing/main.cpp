	...
	namespace SSDServing {
		...
		int BootProgram(bool forANNIndexTestTool, 
			...
			std::string QuantizerFilePath = "";
      		LOG(Helper::LogLevel::LL_Info, "forANNIndexTestTool=%d,configurationPath = %s\n", forANNIndexTestTool, configurationPath);
			if (forANNIndexTestTool) {
        		...
        		LOG(Helper::LogLevel::LL_Info, "config file path = %s\n", configurationPath);
        		LOG(Helper::LogLevel::LL_Info, "valuetype = %s\n", (*config_map)[SEC_BASE]["valuetype"].c_str());
			...
			LOG(Helper::LogLevel::LL_Info, "Set QuantizerFile = %s\n", QuantizerFilePath.c_str());
      		//LOG(Helper::LogLevel::LL_Info, "variable valueType = %s\n", valueType);

			std::shared_ptr<VectorIndex> index = VectorIndex::CreateInstance(IndexAlgoType::SPANN, valueType/*VectorValueType::Float*/);
...
#undef DefineVectorValueType
      ...
      //LOG(Helper::LogLevel::LL_Info, "Configure querySize=%d.\n",opts->m_querySize);
			if (opts->m_generateTruth)
			{
				...
				auto querySet = queryReader->GetVectorSet(0, opts->m_querySize);
				if (distCalcMethod == DistCalcMethod::Cosine && !index->m_pQuantizer) vectorSet->Normalize(opts->m_iSSDNumberOfThreads);
        
        LOG(Helper::LogLevel::LL_Info, "vector size:%d, query size:%d \n", vectorSet->Count(), querySet->Count());
...
#undef DefineVectorValueType
      ...
      LOG(Helper::LogLevel::LL_Info, "searchSSD=%d.\n", searchSSD);
			if (searchSSD) {
#define DefineVectorValueType(Name, Type) \
	if (opts->m_valueType == VectorValueType::Name) { \
...
#undef DefineVectorValueType
      ...
      //LOG(Helper::LogLevel::LL_Info, "SSD Finished.\n");
			return 0;
		}
	}
}
