...
std::vector<SizeType> DefaultVectorReader::GetVectorSplitSet()
{
    ...
    if (ptr->ReadBinary(sizeof(SizeType), (char*)&row) != sizeof(SizeType)) {
        LOG(Helper::LogLevel::LL_Error, "Failed to read SplitVectorSet!\n");
        throw std::runtime_error("Failed read file");
    }

    baseVectorSplitList.resize(row);
    for (int i = 0; i < row; i++) {
        if ( ptr->ReadBinary(4, (char*)&baseVectorSplitList[i] ) != 4) {
            LOG(Helper::LogLevel::LL_Error, "Failed to read split set!\n");
            throw std::runtime_error("Failed read file");
        }
    }

    return baseVectorSplitList;
}
